...
object EpochDeadline extends Ordering[EpochDeadline] {
  ...
  implicit val deadlineAccessor =
    JSONAccessor.create[EpochDeadline, JNumber](
      { deadline =>
        val mils = deadline.time.toMillis

        JNumber(mils / 1000.0)
      },
      { jv =>
        val num = jv.toJNumber
        require(!num.isNaN, "bad number " + jv)
